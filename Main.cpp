...
//LIBRERIAS QUE HE A�ADIDO YO(RLX) PARA HECER CONCEPT OBJECTS Y LLAMADAS AL SISTEMA SON: <objbase.h> <shlwapi.h> <shlobj.h> <windows.h> <tchar.h>
...
void createalumno(char **argv) {//ESto son los create alumno en ello tenemos los 4 parametros que pasa el servidor sql 
	...
	alumno[contadoralumnoscreados] = new Alumno{ par[0], par[1], par[3], grado };//Creamos la clase enviando los parametros aqui tenemos un problema y es que grado no es una string como tal por lo tanto tendremos que a�adirlo en un futuro con una clave foranea
	cout << "es" << par[3] << endl;
	contadoralumnoscreados++;//A�adimos 1 al contador de los alumnos creados para que la proxima vez que a�adamos un nuevo alumno se cree en el siguiente string y no se sobreescriva
...
void insertarclasealumno() {//Esto es para crear la clase que a su vez llamara a la funcion que temos antes que sera la callback
	sql = "SELECT * from ALUMNOS"; //Esto sirve para seleccionar todom, si solo queremos por ejemplo a�adir los alumnos que se llamen  jacinto en vez de select * pondremos select * where nombre=alumno
	tipocreate = 1;
